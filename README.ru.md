...
- **24 скилла** для реальной геймдев-работы: мерджи сцен и префабов, EditMode/PlayMode тесты, разбор IL2CPP-сборок, профилирование с бюджетами, автоматизация редактора через MCP, поэтапные апгрейды.
...
| `unity-debug` | Отладка до первопричины: воспроизвести, локализовать, починить, защитить |
| `unity-review` | Ревью Unity-диффов уровня владельца кода с классификацией серьёзности |
...
| `arch-audit` | Аудит архитектуры модуля -> упорядоченный по зависимостям бэклог рефакторинга |
...
C# бэкенд (`backend-...`), для игровых серверов и сервисов: `backend-orient`, `backend-implement`, `backend-debug`, `backend-review`, `backend-validate` - та же дисциплина в ASP.NET-вкусе, ставится бэкенд-шаблоном.
...
Установка кладёт в корень проекта живые контракты, которые держат агентов в колее: `AGENTS.md` (стек, карта модулей, границы, роутинг скиллов), `ARCHITECTURE.md` (форма модулей, границы asmdef, детерминизм, совместимость сериализации), `CODE_STYLE.md`, `DEPENDENCIES.md` (каждый пакет обоснован). Агенты читают их до действий; `learn` копит уроки проекта в `.agents/learnings.md`.
...
| --- | --- | --- | --- | --- |
| Инструкции | `AGENTS.md` + контракты | нативно | через `CLAUDE.md` | нативно |
...
Обновите пакет в Package Manager - окно Agent Kit предложит **Update**: обновит немодифицированные файлы, сохранит каждую вашу правку с пометкой `KEEP`, удалит файлы, которые кит больше не поставляет. **Uninstall** удаляет только немодифицированные файлы кита. **Dry run** показывает любую операцию без записи. Скриптовые установки используют тот же манифест - режимы можно свободно смешивать.

<details>
<summary><b>Другие способы установки (PowerShell, бэкенд-шаблон, глобальный профиль, плагин)</b></summary>

Все скриптовые установщики поддерживают `-Update`, `-Force` и `-WhatIf` с той же семантикой манифеста.
...
Та же форма: бэкенд-контракты, 11 backend+общих скиллов, рендеренные адаптеры. Цель должна содержать `.sln`, `.slnx` или `.csproj`.
...
Ставит профиль `unity-codex` в `~/.codex` (запуск: `codex --profile unity-codex`). `-InstallAgentsMd` активирует полную глобальную дисциплину из 18 разделов (существующий файл бэкапится), `-InstallSkills` копирует все 24 скилла в пользовательский скоуп, `-InstallClaude` добавляет глобальный слой Claude Code, `-InstallWslSkills` покрывает Codex под WSL.
...
# Самопроверка кита: frontmatter, ссылки, манифесты, наборы скиллов, дрейф адаптеров и UPM-payload, ASCII-политика.
...
plugins/
  codex-unity-agent-kit/   плагин: 24 скилла (единый источник правды)
