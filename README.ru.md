...
Три режима: **stage** (по умолчанию - один этап и стоп), **milestone** (этапы 2-7 без пауз между зелёными гейтами), **auto** (цикл по майлстоунам, пока не выполнен MVP-чеклист из GDD). Состояние пайплайна живёт в `.agents/plans/pipeline.md`, дизайн-контракт - в `docs/design/game-design.md`: любой агент на любой платформе продолжает работу из файлов, а не из памяти чата.
