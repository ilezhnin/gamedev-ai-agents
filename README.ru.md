...
Три режима: **stage** (по умолчанию - один этап и стоп), **milestone** (этапы 2-7 без пауз между зелёными гейтами), **auto** (цикл по майлстоунам, пока не выполнен MVP-чеклист из GDD). Режимы milestone и auto включаются только вашей явной командой, а пайплайн требует контракт: без утверждённого GDD он сначала предложит `gdd`. Состояние пайплайна живёт в `.agents/plans/pipeline.md`, дизайн-контракт - в `docs/design/game-design.md`: любой агент на любой платформе продолжает работу из файлов, а не из памяти чата.
...
Reasoning ролей распределён по ответственности: планирование, архитектура, геймдизайн, проверка консистентности и production coordination рендерятся как Codex `xhigh` и Claude Code `max`; ревьюеры, генерация ассетов и release engineering используют `high`; роли исполнения, сборки контекста, поиска/интеграции ассетов, исследования, валидации, QA и shipping остаются на `medium`. Gemini CLI и Antigravity получают общее проектное поведение, но кит не закрепляет там model choice для каждой роли.

Codex, Claude Code и Gemini CLI получают рабочие хуки для проверки гигиены `.meta`/GUID после правок и отчёта об использовании после хода. Usage-хуки пишут `.agents/usage/last-report.md` и history; поскольку часть клиентов скрывает hook-сообщения, установленный `AGENTS.md` также требует добавлять видимую usage-статистику из `.agents/scripts/usage-footer.ps1` в каждый final response. Закомментированный блок `[mcp_servers.unity]` в `.codex/config.toml` показывает, куда подключить сервер [MCP for Unity](https://github.com/CoplayDev/unity-mcp) - с ним `unity-mcp` и QA-этап пайплайна управляют редактором напрямую: сцены, PlayMode, тесты, скриншоты. Claude Code читает MCP-серверы из `.mcp.json`; правило-указатель для Cursor тоже в комплекте.
