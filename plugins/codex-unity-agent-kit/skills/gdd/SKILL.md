...
11. Write or update one slug-scoped GDD using `references/gdd-format.md`. Create `.agents/plans/.gitignore` with `*` and `!.gitignore` when missing. Never overwrite another feature's GDD.
...
- The selected GDD is a living working contract: update its scoped file in place when the user approves a changed decision; move discarded scope to Later / Cut. Pipeline approval is bound to the exact content hash.
