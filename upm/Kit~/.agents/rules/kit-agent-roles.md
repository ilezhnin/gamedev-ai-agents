...
- Before committing or amending, verify git config user.name/user.email plus GIT_AUTHOR_IDENT and GIT_COMMITTER_IDENT; stop on missing, root, root@..., .localdomain, or machine fallback identity. Use Conventional Commits in English, atomic commits, and descriptive branch names (feat/fix/chore). Never add AI attribution anywhere.
