...
- `global/canon/` (roles.json, permissions.json, hooks.json) is the single source of truth for agent roles, permissions, and hooks. The platform adapter layers - `.codex/agents/*.toml` + `.codex/rules` + `.codex/hooks.json`, `.claude/agents/*.md` + `.claude/settings.json`, and the `.agents/rules/*.md` Antigravity rules - are rendered from the canon at install time and must never be stored in the kit (`scripts/validate-kit.ps1` forbids the stored copies). The `reasoning` field currently renders only to Codex (`model_reasoning_effort`); Claude Code and Antigravity have no per-agent equivalent, so it is intentionally dropped there.
